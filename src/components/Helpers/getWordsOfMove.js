  ...
  for(let i=0;i<candidatesForMove.length;i++){
    ...
    if(verticalTip2-verticalTip1!=2*widthAndLengthOfBoard){
      for (let j = verticalTip1+widthAndLengthOfBoard; j < verticalTip2; j = j + widthAndLengthOfBoard) {
        if ( candidatesForMove.find((letter) => letter.position == j) != undefined) {
          verticalWord.letters.push({ position: Board[j],letter:cells[j],id:j,countAsZero:candidatesForMove.find((letter) => letter.position == j).countAsZero });
    ...
    if(horizontalTip2-horizontalTip1!=2){
      for (let j = horizontalTip1+1; j < horizontalTip2; j = j + 1) {
        if ( candidatesForMove.find((letter) => letter.position == j) != undefined) {
          horizontalWord.letters.push({ position: Board[j],letter:cells[j],id:j,countAsZero:candidatesForMove.find((letter) => letter.position == j).countAsZero });
        }else{
